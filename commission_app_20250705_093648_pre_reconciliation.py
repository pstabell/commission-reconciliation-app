...
def check_password():
    ...
    def password_entered():
        ...
        # Debug: Show what password is expected (remove this after debugging!)
        st.write(f"DEBUG: App is expecting password from env: {correct_password[:3]}...{correct_password[-3:]}")
        st.write(f"DEBUG: Full password length: {len(correct_password)} characters")
        st.write(f"DEBUG: ENV file loaded: {os.path.exists('.env')}")
