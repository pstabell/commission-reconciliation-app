                        ...
                        if view_mode != "Aggregated by Policy" and 'Policy Number' in editable_data.columns:
                            ...
                            for group_name in unique_groups:
                                ...
                                if len(group_df) > 0:
                                    ...
                                    # 1. NEW or RWL transactions (policy start)
                                    is_new_rwl = group_df['Transaction Type'].isin(['NEW', 'RWL'])
                                    new_rwl_df = group_df[is_new_rwl]
                                    
                                    # 2. END transactions
                                    is_end = group_df['Transaction Type'] == 'END'
                                    end_df = group_df[is_end].sort_values('Effective Date')
                                    
                                    # 3. STMT/VOID transactions (sort by STMT DATE)
                                    ...
                                    # 4. Other transactions (not NEW/RWL/END/STMT/VOID)
                                    is_other = ~(is_new_rwl | is_end | is_stmt_void)
                                    other_df = group_df[is_other]
