                ...
                with st.expander("🙈 Hidden Policies" if view_mode == "Aggregated by Policy" else "🙈 Hidden Transactions", expanded=True):
                    ...
                    if len(hidden_df) > 0:
                        ...
                        if view_mode == "Aggregated by Policy":
                            ...
                            # Transaction-level columns for detailed view
                            display_cols = ['Transaction ID', 'Customer', 'Policy Number', 'Policy Type', 'Transaction Type', 
                                          'Total Agent Comm', 'Agent Paid Amount (STMT)', 'Policy Balance Due']
                        
                        # Use Total Agent Comm if available, otherwise fall back to Agent Estimated Comm $
                        if 'Total Agent Comm' not in hidden_df.columns and 'Agent Estimated Comm $' in hidden_df.columns:
                            display_cols = [col if col != 'Total Agent Comm' else 'Agent Estimated Comm $' for col in display_cols]
                ...
                with st.expander("✅ Reviewed Policies" if view_mode == "Aggregated by Policy" else "✅ Reviewed Transactions", expanded=True):
                    if 'prl_reviewed_rows' in st.session_state and st.session_state.prl_reviewed_rows:
                        ...
                        if len(reviewed_df) > 0:
                            ...
                            # Use Total Agent Comm if available, otherwise fall back to Agent Estimated Comm $
                            if 'Total Agent Comm' not in reviewed_df.columns and 'Agent Estimated Comm $' in reviewed_df.columns:
                                display_cols = [col if col != 'Total Agent Comm' else 'Agent Estimated Comm $' for col in display_cols]
