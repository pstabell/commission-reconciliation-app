...
def match_statement_transactions(statement_df, column_mapping, existing_data, statement_date):
    ...
    for idx, row in statement_df.iterrows():
        ...
        amount = float(row[column_mapping['Agency Comm Received (STMT)']]) if 'Agency Comm Received (STMT)' in column_mapping else 0
                ...
                if uploaded_file is not None:
                    ...
                    try:
                        ...
                        required_fields = {
                            ...
                            'Agency Comm Received (STMT)': 'Commission Amount'
                        }
                ...
                if not recon_entries.empty:
                    ...
                    if not filtered_recon.empty:
                        ...
                        if view_mode == "By Batch" and 'reconciliation_id' in filtered_recon.columns:
                            # Show batch summary
                            batch_summary = filtered_recon.groupby('reconciliation_id').agg({
                                'Transaction ID': 'count',
                                'Agency Comm Received (STMT)': 'sum',
                                'STMT DATE': 'first'
                            }).reset_index()
                            
                            batch_summary = batch_summary.rename(columns={
                                'reconciliation_id': 'Batch ID',
                                'Transaction ID': 'Transaction Count',
                                'Agency Comm Received (STMT)': 'Batch Total',
                                'STMT DATE': 'Statement Date'
                            })
                            ...
                            st.dataframe(
                                ...
                                column_config={
                                    "Batch Total": st.column_config.NumberColumn(format="$%.2f")
                                },
                                use_container_width=True,
                                hide_index=True
                            )
                            ...
                            st.dataframe(
                                display_recon[[
                                    'Transaction ID', 'Customer', 'Policy Number', 
                                    'STMT DATE', 'Agency Comm Received (STMT)'
