        ...
        if selected_policy and selected_policy != "Select...":
            ...
            if not ledger_df.empty:
                    ...
                    if pd.notna(effective_date) and effective_date != "N/A":
                        try:
                            effective_date = pd.to_datetime(effective_date).strftime("%B %d, %Y")
                        except:
                            pass
                    ...
                    if pd.notna(x_date) and x_date != "N/A":
                        try:
                            x_date = pd.to_datetime(x_date).strftime("%B %d, %Y")
                        except:
                            pass
                    ...
                    with date_col1:
                        st.markdown(
                            ...
                            <div style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; height: 120px;">
                                <div style="color: #666; font-size: 14px; margin-bottom: 5px;">📅 Policy</div>
                                <div style="color: #666; font-size: 14px; margin-bottom: 10px;">&nbsp;&nbsp;&nbsp;&nbsp;Origination Date</div>
                    ...
                    with date_col3:
                        st.markdown(
                            f"""
                            <div style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; height: 120px;">
                                <div style="color: #666; font-size: 14px; margin-bottom: 5px;">🔄 Expiration Date</div>
                                <div style="color: #666; font-size: 14px; margin-bottom: 10px;">&nbsp;&nbsp;&nbsp;&nbsp;(X-DATE)</div>
                    ...
                    # Store the data in session state for editing
                    if 'policy_details_data' not in st.session_state:
                        st.session_state.policy_details_data = policy_details_row.to_dict() if not policy_details_row.empty else {}
                    
                    # Edit button
                    st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)
                    if st.button("✏️ Edit Details", key="edit_policy_details_btn"):
                        st.session_state.editing_policy_details = True
                    
                    # Show edit form if in editing mode
                    if st.session_state.get('editing_policy_details', False):
                        st.divider()
                        st.markdown("### ✏️ Edit Policy Details")
                        
                        # Create form for editing
                        with st.form("policy_details_form"):
                            col1, col2 = st.columns(2)
                            
                            with col1:
                                edited_customer = st.text_input("Customer", value=customer if customer != "N/A" else "")
                                edited_policy_number = st.text_input("Policy Number", value=policy_number if policy_number != "N/A" else "")
                                edited_carrier = st.text_input("Carrier Name", value=carrier_name if carrier_name != "N/A" else "")
                                edited_effective = st.date_input("Effective Date", value=pd.to_datetime(policy_details_row.get("Effective Date")) if pd.notna(policy_details_row.get("Effective Date")) else None)
                                edited_gross_comm = st.number_input("Gross Commission %", value=float(gross_comm) if gross_comm else 0.0, step=0.01)
                            
                            with col2:
                                edited_client_id = st.text_input("Client ID", value=client_id if client_id != "N/A" else "")
                                edited_policy_type = st.text_input("Policy Type", value=policy_type if policy_type != "N/A" else "")
                                edited_mga = st.text_input("MGA Name", value=mga_name if mga_name != "N/A" else "")
                                edited_x_date = st.date_input("Expiration Date (X-DATE)", value=pd.to_datetime(policy_details_row.get("X-DATE")) if pd.notna(policy_details_row.get("X-DATE")) else None)
                                edited_agent_comm = st.number_input("Agent Commission %", value=float(agent_comm) if agent_comm else 0.0, step=0.01)
                            
                            # Origination date (full width)
                            edited_origination = st.date_input("Policy Origination Date", value=pd.to_datetime(policy_details_row.get("Policy Origination Date")) if pd.notna(policy_details_row.get("Policy Origination Date")) else None)
                            ...
                            if save_button:
                                ...
                                update_data = {
                                    ...
                                    "Client ID": edited_client_id,
                                    "Policy Number": edited_policy_number,
                                    "Policy Type": edited_policy_type,
                                    "Carrier Name": edited_carrier,
                                    "MGA Name": edited_mga,
                                    "Effective Date": edited_effective,
                                    "Policy Origination Date": edited_origination,
                                    "Policy Gross Comm %": edited_gross_comm,
                                    "Agent Comm (NEW 50% RWL 25%)": edited_agent_comm,
                                    "X-DATE": edited_x_date
                                }
                                
                                # Save the changes
                                st.session_state.policy_details_to_save = update_data
                                st.session_state.editing_policy_details = False
                                st.rerun()
                    ...
                    # Handle saving (outside the form)
                    if 'policy_details_to_save' in st.session_state:
                        update_data = st.session_state.policy_details_to_save
                        del st.session_state.policy_details_to_save
                        
                        try:
                            # Update via Supabase
                            if not policy_rows.empty and '_id' in policy_rows.columns:
                                policy_id = policy_rows.iloc[0]['_id']
                            else:
                                # Get the record ID first
                                result = supabase.table('policies').select('_id').eq('Policy Number', update_data['Policy Number']).execute()
                                if result.data:
                            ...
                            if policy_id:
                                # Remove Policy Number from update (it's the key)
                                update_dict = {k: v for k, v in update_data.items() if k not in ['_id', 'Policy Number']}
                                supabase.table('policies').update(update_dict).eq('_id', policy_id).execute()
                                clear_policies_cache()
                                st.success("✅ Policy details updated successfully!")
                                time.sleep(1)
                                st.rerun()
                            else:
                                st.error("Policy not found in database")
                        except Exception as e:
                    ...
                    column_config["Type"] = {
                        "disabled": True,
                        "help": "Transaction type indicator: 💙=STMT (reconciliation), 🔴=VOID, 📄=Regular",
                        "width": "small"
                    }
