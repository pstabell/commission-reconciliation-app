    ...
    for idx, row in statement_df.iterrows():
        ...
        if potential_customers:
            ...
            if customer_key in customer_trans_lookup:
                ...
                # Check for amount match WITH policy number verification
                amount_matched = False
                for trans in customer_matches:
                    if amount > 0 and abs(trans['balance'] - amount) / amount <= 0.05:  # 5% tolerance
                        ...
                        if trans.get('Policy Number') == policy_num and policy_num:
                            match_result['match'] = trans
                            match_result['confidence'] = 90
                            match_result['match_type'] = 'Customer + Policy + Amount'
                            matched.append(match_result)
                            amount_matched = True
                            break
...
def show_import_results(statement_date, all_data):
    ...
    if has_manual_matches and not (has_matched or has_to_create):
        st.info("💡 You have pending manual matches. Click 'Apply Manual Matches' above before importing, or the import button will process them automatically.")
    
    if st.button("🔄 Proceed with Import", type="primary", disabled=not can_import):
            ...
            try:
                            ...
                            if 'client_action' in item:
                                if item['client_action'] == 'existing' and item.get('client_id'):
                                    ...
                                    try:
                                        ...
                                        # Silently skip client creation if table doesn't exist
                                        pass  # Client ID feature not available
                            else:
                ...
                for item in st.session_state.matched_transactions:
                    ...
                    recon_entry.update({
                        ...
                        'Client ID': item['match'].get('Client ID', '') if 'match' in item and item['match'] else '',
    ...
    with st.form("edit_transaction_form"):
        ...
        with col6:
            ...
            if 'X-DATE' in modal_data.keys():
                ...
                # Only use calculated X-DATE if user explicitly changed Policy Term
                # Check if we have a calculated X-DATE from Policy Term selection
                if 'calculated_x_date' in st.session_state and st.session_state.get('user_changed_policy_term', False):
                    date_value = st.session_state['calculated_x_date']
                # else: Keep the existing X-DATE value
                ...
                if date_value and pd.notna(date_value):
                    try:
                        ...
                        updated_data['X-DATE'] = st.date_input(
                            ...
                            value=parsed_date.date(),
                            key="modal_X-DATE",
                            help="💡 When you manually enter an X-DATE, the Policy Term will be automatically calculated when you click the Calculate button"
                        )
                    except:
                        updated_data['X-DATE'] = st.text_input(
                            ...
                            value=str(date_value),
                            key="modal_X-DATE",
                            help="Enter date in MM/DD/YYYY format. 💡 When you manually enter an X-DATE, the Policy Term will be automatically calculated when you click the Calculate button"
                        )
                else:
                    updated_data['X-DATE'] = st.date_input(
                        'X-DATE (Expiration)',
        ...
        with col6:
            if 'Policy Term' in modal_data.keys():
                ...
                # Only calculate X-DATE if the user explicitly changed the Policy Term
                # Check if the Policy Term was changed from its original value
                original_term = modal_data.get('Policy Term')
                user_changed_term = False
                
                # Check if user actually changed the term
                if 'modal_Policy Term' in st.session_state:
                    # Compare current selection with original value
                    if selectbox_value != original_term:
                        user_changed_term = True
                        st.session_state['user_changed_policy_term'] = True
                    else:
                        st.session_state['user_changed_policy_term'] = False
                
                # Only update X-DATE if user explicitly changed the Policy Term
                if user_changed_term and selectbox_value and selectbox_value != "Custom" and effective_date:
                    try:
                        ...
                        st.info(f"💡 X-DATE will be set to {new_x_date.strftime('%Y-%m-%d')} (Effective Date + {selectbox_value} months)")
                    except Exception as e:
                        st.warning(f"Could not calculate X-DATE: {str(e)}")
                elif user_changed_term and selectbox_value == "Custom":
                    st.info("📝 Custom term selected - please enter X-DATE manually. The Policy Term will be calculated based on your X-DATE when you click Calculate.")
        ...
        with col_calc1:
            existing_premium = st.number_input(
                "Existing Net Premium", 
                value=0.00, 
                format="%.2f", 
                step=100.0,
                key="modal_existing_premium",
                help="Enter the current/existing net premium amount"
            )
        with col_calc2:
            new_premium = st.number_input(
                "New/Revised Gross Premium", 
                value=0.00, 
                format="%.2f", 
                step=100.0,
                key="modal_new_premium",
                help="Enter the new/revised gross premium amount"
            )
        with col_calc3:
        ...
        with col11:
            if 'Agent Comm %' in modal_data.keys():
                ...
                # Check if this should be editable (CAN transaction after Calculate clicked)
                is_editable = (current_transaction_type == "CAN" and 
                              st.session_state.get('calculate_clicked', False))
                
                # Display field with appropriate settings
                ...
                if is_editable:
                    # For CAN transactions after Calculate, make it editable
        ...
        with col12:
            ...
            # Use the actual value from updated_data to respect manual overrides
            agent_comm_pct = updated_data.get('Agent Comm %', agent_rate if 'agent_rate' in locals() else 0)
            try:
        ...
        if save_button:
            ...
            # Remove the auto-override of Policy Term - respect user's selection
            # The auto-calculation now happens in the form display logic
            # and the user can override it if needed
...
def main():
    ...
    # --- All Policy Transactions ---
        ...
        if all_data.empty:
            ...
            preferred_order = [
                ...
                'Policy Origination Date', 'Effective Date', 'X-DATE', 'Policy Term',  # X-DATE moved before Policy Term
                'Premium Sold', 'Policy Taxes & Fees', 'Commissionable Premium',
            ...
            if edit_search_term or edit_search_button or show_attention_filter:
                ...
                if 'edit_results' in locals() and not edit_results.empty:
                    ...
                    if not edit_results.empty:
                        ...
                        # Add Carrier Name, MGA Name, Policy Type, Transaction Type before Policy Number
                        carrier_related_cols = ['Carrier Name', 'MGA Name', 'Policy Type', 'Transaction Type']
                        for col in carrier_related_cols:
                            if col in all_cols:
                        ...
                        # Add Prior Policy Number right after Policy Number
                        if 'Prior Policy Number' in all_cols:
                            priority_cols.append('Prior Policy Number')
                        
                        # Add the date columns and Policy Term in the correct order
                        date_cols_ordered = ['Policy Origination Date', 'Effective Date', 'X-DATE', 'Policy Term']
                        for col in date_cols_ordered:
                            if col in all_cols:
                                priority_cols.append(col)
                        
                        # Now add financial columns in specific order
                        
                        # Premium and Commission columns
                        if 'Premium Sold' in all_cols:
                            priority_cols.append('Premium Sold')
                        
                        # Move Broker Fee, Policy Taxes & Fees, Commissionable Premium before Agency Estimated Comm/Revenue
                        financial_order_1 = ['Broker Fee', 'Policy Taxes & Fees', 'Commissionable Premium']
                        for col in financial_order_1:
                            if col in all_cols:
                                priority_cols.append(col)
                        
                        # Agency columns
                        if 'Policy Gross Comm %' in all_cols:
                            priority_cols.append('Policy Gross Comm %')
                        if 'Agency Estimated Comm/Revenue (CRM)' in all_cols:
                            priority_cols.append('Agency Estimated Comm/Revenue (CRM)')
                        if 'Agency Comm Received (STMT)' in all_cols:
                            priority_cols.append('Agency Comm Received (STMT)')
                        
                        # Agent columns - Move Agent Comm % before Agent Estimated Comm $
                        if 'Agent Comm %' in all_cols:
                            priority_cols.append('Agent Comm %')
                        if 'Agent Estimated Comm $' in all_cols:
                            priority_cols.append('Agent Estimated Comm $')
                        
                        # Broker fee commission and total
                        if 'Broker Fee Agent Comm' in all_cols:
                            priority_cols.append('Broker Fee Agent Comm')
                        if 'Total Agent Comm' in all_cols:
                            priority_cols.append('Total Agent Comm')
                        
                        # Payment tracking
                        if 'Agent Paid Amount (STMT)' in all_cols:
                            priority_cols.append('Agent Paid Amount (STMT)')
                        
                        # Move STMT DATE after Agent Paid Amount (STMT)
                        if 'STMT DATE' in all_cols:
                            priority_cols.append('STMT DATE')
                        
                        # Balance
                        if 'Policy Balance Due' in all_cols:
                            priority_cols.append('Policy Balance Due')
                        
                        # Add any remaining columns that aren't in priority list
                        ...
                        # Get client ID if all rows have the same client
                        if client_id_col:
                            ...
                            if len(unique_client_ids) == 1:
                                ...
                                if existing_customer_name:
                                    st.info(f"💡 New rows will be created for {existing_customer_name} (Client ID: {existing_client_id})")
                                else:
                        ...
                        # Calculate selected count once for all buttons
                        selected_count = 0
                        selected_idx = None
                        if 'Select' in edited_data.columns:
                            ...
                            if (prev_selected_key not in st.session_state or 
                                ...
                                if selected_count == 1:
                                    ...
                                    selected_idx = edited_data[selected_mask].index[0]
                                    st.session_state[f"{editor_key}_selected_idx"] = selected_idx
                            else:
                                # Use cached values
                        ...
                        # Add buttons for adding new row, duplicating, and editing selected row
                        ...
                        # Add Duplicate button in the middle column
                        with button_col2:
                            # Check for selected rows for duplicate button
                            if 'Select' in edited_data.columns:
                                if selected_count == 1:
                                    if st.button("📋 Duplicate Selected Transaction", type="secondary", use_container_width=True):
                                        # Get the selected transaction data
                                        if selected_idx is not None:
                                            # Get the transaction data
                                            transaction_data = edited_data.loc[selected_idx].to_dict()
                                            ...
                                            # Create a new transaction data dict without excluded fields
                                            duplicate_data = {}
                                    ...
                                    st.button("📋 Duplicate Selected Transaction", type="secondary", use_container_width=True, disabled=True, help=f"{selected_count} selected - please select only ONE transaction")
                            else:
                                st.button("📋 Duplicate Selected Transaction", type="secondary", use_container_width=True, disabled=True, help="No selection column available")
                        ...
                        if st.session_state.get('show_edit_modal', False):
                            ...
                            if is_duplicate_mode:
                                ...
                                st.info("🆕 Creating a new transaction with copied data. A new Transaction ID will be generated.")
                            else:
                            ...
                            if result:
                                if result["action"] == "save" or result["action"] == "duplicate":
                                    try:
                                        ...
                                        is_duplicate_action = result["action"] == "duplicate"
                                        ...
                                        if transaction_id and not is_duplicate_action:  # Don't check for existing if duplicating
                                            try:
                                        ...
                                        if is_duplicate_action:
                                            # Duplicate action - always INSERT a new record
    ...
    # --- Add New Policy Transaction ---
        ...
        with st.form("add_policy_form"):
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                existing_premium = st.number_input("Existing Net Premium", value=0.00, format="%.2f", step=100.0)
            with col2:
                new_premium = st.number_input("New/Revised Gross Premium", value=0.00, format="%.2f", step=100.0)
            with col3:
                premium_sold_calc = new_premium - existing_premium
    ...
    # --- Reconciliation ---
        ...
        # Store the current tab for persistence
        if 'reconciliation_active_tab' not in st.session_state:
            st.session_state.reconciliation_active_tab = 0
        
        rec_tab1, rec_tab2, rec_tab3, rec_tab4 = st.tabs(tab_names)
        
        with rec_tab1:
            # Mark this tab as active
            st.session_state.reconciliation_selected_tab = "Import Statement"
                ...
                if not recon_entries.empty:
                    ...
                    if not filtered_recon.empty:
                        ...
                        if view_mode == "By Batch" and 'reconciliation_id' in filtered_recon.columns:
                            ...
                            display_columns = ['Edit', 'Transaction ID', 'STMT DATE', 'Transaction Type', 'Customer', 'Client ID', 'Policy Number', 'Policy Type', 'Carrier Name', 'Effective Date', 'X-DATE']
                            
                            # Add commission percentage column
                            if 'Agent Comm %' in display_recon.columns:
                                display_columns.append('Agent Comm %')
                                ...
                                if len(selected_for_edit) > 1:
                                    ...
                                    with st.form("edit_reconciled_transaction_form"):
                                        ...
                                        with form_col2:
                                            ...
                                            # Add Client ID as editable field
                                            new_client_id = st.text_input(
                                                f"Client ID *(was: {original_data.get('Client ID', '')})*",
                                                value=original_data.get('Client ID', ''),
                                                key="edit_new_client_id",
                                                help="Enter the Client ID to properly link this transaction"
                                            )
                                            ...
                                            # Add the new editable fields
                                            new_policy_type = st.text_input(
                                                f"Policy Type *(was: {original_data.get('Policy Type', '')})*",
                                                value=original_data.get('Policy Type', ''),
                                                key="edit_new_policy_type"
                                            )
                                            
                                            new_carrier_name = st.text_input(
                                                f"Carrier Name *(was: {original_data.get('Carrier Name', '')})*",
                                                value=original_data.get('Carrier Name', ''),
                                                key="edit_new_carrier_name"
                                            )
                                            
                                            # Handle effective date formatting
                                            eff_date_value = original_data.get('Effective Date', '')
                                            if pd.notna(eff_date_value):
                                                try:
                                                    # Convert to date object for date_input
                                            ...
                                            new_effective_date = st.date_input(
                                                f"Effective Date *(was: {formatted_eff_date})*",
                                                value=eff_date_obj,
                                            ...
                                            new_policy_origination_date = st.date_input(
                                                f"Policy Origination Date *(was: {formatted_pol_orig_date})*",
                                                value=pol_orig_date_obj,
                                                key="edit_new_policy_origination_date"
                                            )
                                            
                                            # X-DATE (Expiration Date)
                                            x_date_value = original_data.get('X-DATE', '')
                                            if pd.notna(x_date_value):
                                                try:
                                            ...
                                            new_x_date = st.date_input(
                                                f"X-DATE (Expiration) *(was: {formatted_x_date})*",
                                                value=x_date_obj,
                                                key="edit_new_x_date"
                                            )
                                        ...
                                        if submitted:
                                            ...
                                            try:
                                                ...
                                                update_data = {
                                                    ...
                                                    'Policy Type': new_policy_type,
                                                    'Carrier Name': new_carrier_name,
                    ...
                    if not reconciliation_entries.empty:
                        ...
                        if batch_options:
                            ...
                            if selected_batch:
                                ...
                                # Check if batch has already been voided
                                already_voided = all_data[
                                    all_data['reconciliation_id'] == f"VOID-{selected_batch}"
                                ].shape[0] > 0
                                
                                if already_voided:
                                    st.error("⚠️ This batch has already been voided. You cannot void a batch twice.")
                                    st.info("If you need to make corrections, please create an adjustment entry instead.")
                                else:
                                    # Void confirmation
                                    st.divider()
                                    
                                    # Apply yellow styling to void reason field
                                    st.markdown("""
                                    <style>
                                        /* Style the void reason text area with app's standard yellow theme */
                                            ...
                                            if st.button("🗑️ Void Batch", type="secondary"):
                                                try:
                                                    ...
                                                    for idx, row in batch_details.iterrows():
                                                        # Extract base ID from original transaction ID
                                                        original_trans_id = row.get('Transaction ID', '')
                                                        if '-STMT-' in original_trans_id:
                                                            # Extract base ID (everything before -STMT-)
                                                            base_id = original_trans_id.split('-STMT-')[0]
                                                            # Create void ID using same base
                                                            void_id = f"{base_id}-VOID-{statement_date.strftime('%Y%m%d')}"
                                                        else:
                                                            # Fallback to generating new ID if pattern doesn't match
...
- Outstanding balance tracking
                ...
                st.code("""
Formula: New/Revised Gross Premium - Existing Net Premium = Premium Sold
Purpose: Calculate the additional premium for endorsements
Example: $1,350 - $1,200 = $150

Fields Used:
- INPUT: New/Revised Gross Premium (user enters)
- INPUT: Existing Net Premium (user enters)
...
-- /sql_scripts/create_commission_structure_tables.sql
            ...
            if selected_mga:
                ...
                if st.session_state.get('editing_mga'):
                    ...
                    with st.form("edit_mga_form"):
                        ...
                        contact_info = selected_mga.get('contact_info', {})
                        with col1:
                            edited_name = st.text_input("MGA Name*", value=selected_mga['mga_name'])
                            edited_contact = st.text_input("Contact Name", value=contact_info.get('contact_name', ''))
                            edited_email = st.text_input("Email", value=contact_info.get('email', ''))
                        
                        with col2:
                            edited_phone = st.text_input("Phone", value=contact_info.get('phone', ''))
                        ...
                        with form_col1:
                            if st.form_submit_button("💾 Save Changes", type="primary"):
                                if edited_name:
                                    try:
                                        # Build contact_info JSONB object
                                        contact_info = {}
                                        if edited_contact:
                                            contact_info['contact_name'] = edited_contact
                                        if edited_phone:
                                            contact_info['phone'] = edited_phone
                                        if edited_email:
                                            contact_info['email'] = edited_email
                    ...
                    with col1:
                        if contact_info.get('contact_name'):
                            st.markdown(f"**Contact:** {contact_info['contact_name']}")
    ...
    # --- Policy Revenue Ledger ---
        ...
        with col_refresh:
            if st.button("🔄 Refresh", help="Refresh data from database"):
                st.cache_data.clear()
        ...
        # Get unique policy numbers with proper grouping
        # Group by Client ID + Policy Number to avoid duplicates from name variations
        if "Client ID" in filtered_data.columns and "Policy Number" in filtered_data.columns:
            # IMPORTANT: Trim whitespace from policy numbers to avoid duplicates
            if "Policy Number" in filtered_data.columns:
                filtered_data["Policy Number"] = filtered_data["Policy Number"].astype(str).str.strip()
            
            # Get unique policy numbers - now properly trimmed
            ...
            st.warning("⚠️ Client ID column not found - using Policy Number only")
        ...
        if selected_policy and selected_policy != "Select...":
            # Get all rows for this policy using Client ID + Policy Number matching
            # This ensures we get all transactions even if customer names vary slightly
            if selected_customer != "Select..." and "Client ID" in all_data.columns:
                ...
                customer_data = all_data_trimmed[
                    (all_data_trimmed["Customer"] == selected_customer) & 
                    (all_data_trimmed["Policy Number"] == selected_policy.strip())
                ]
                if not customer_data.empty and "Client ID" in customer_data.columns:
                    # Get the most common Client ID for this combination
                    client_ids = customer_data["Client ID"].dropna()
                    if not client_ids.empty:
                        client_id = client_ids.mode().iloc[0] if not client_ids.mode().empty else client_ids.iloc[0]
                        # Now get ALL rows with this Client ID + Policy Number combination
                        # Use trimmed data to match properly
                        policy_rows = all_data_trimmed[
                        ...
                        # Add any STMT transactions not already in policy_rows
                        if not stmt_rows.empty:
                            existing_trans_ids = set(policy_rows["Transaction ID"].astype(str))
                            new_stmt_rows = stmt_rows[~stmt_rows["Transaction ID"].astype(str).isin(existing_trans_ids)]
                            if not new_stmt_rows.empty:
                                policy_rows = pd.concat([policy_rows, new_stmt_rows], ignore_index=True)
                                # st.warning(f"⚠️ Found {len(new_stmt_rows)} STMT transactions with missing/different Client IDs")
                        
                        # Debug messages removed - transactions found successfully
                    else:
                ...
                # Fallback to policy number only - need to trim here too
                all_data_trimmed = all_data.copy()
                if "Policy Number" in all_data_trimmed.columns:
                    all_data_trimmed["Policy Number"] = all_data_trimmed["Policy Number"].astype(str).str.strip()
                policy_rows = all_data_trimmed[all_data_trimmed["Policy Number"] == selected_policy.strip()].copy()
            ...
            if not policy_rows.empty and "X-DATE" in policy_rows.columns:
                # Get unique X-DATEs with their transaction types
                xdate_info = []
                for xdate in sorted(policy_rows["X-DATE"].dropna().unique()):
                    # Get transaction types for this X-DATE
                    trans_types = policy_rows[policy_rows["X-DATE"] == xdate]["Transaction Type"].unique()
                    for trans_type in trans_types:
                        # Create formatted display with color coding
                        if trans_type == "CAN":
                            ...
                            display_text = f"{xdate} 📄 {trans_type}"
                        xdate_info.append((xdate, trans_type, display_text))
                
                # Check if policy has multiple transaction types
                unique_trans_types = policy_rows["Transaction Type"].unique()
                has_multiple_types = len(unique_trans_types) > 1
                has_cancellation = any(t in unique_trans_types for t in ["CAN", "XCL"])
                
                # Create filter selection
                st.markdown("### 📅 Policy Term Filter (Optional)")
                
                # Show info message if policy has multiple transaction types
                if has_multiple_types and has_cancellation:
                    st.info("ℹ️ This policy has multiple transactions including cancellations. Showing all terms for complete history.")
                
                term_col1, term_col2, term_col3 = st.columns([2, 2, 2])
                
                with term_col1:
                    # Build options list with formatted X-DATEs
                    x_date_options = ["All Terms"]
                    x_date_mapping = {}  # Map display text back to actual X-DATE
                    
                    for xdate, trans_type, display_text in xdate_info:
                        x_date_options.append(display_text)
                        x_date_mapping[display_text] = xdate
                    
                    # Auto-select "All Terms" if policy has cancellation
                    default_index = 0  # Always default to "All Terms" when there are multiple types
                    
                    selected_xdate_display = st.selectbox(
                        "Select Policy Term by X-DATE:",
                        options=x_date_options,
                        index=default_index,
                        key="ledger_xdate_select",
                        help="Filter to show only transactions for a specific policy term. Transaction types are shown next to dates."
                    )
                    
                    # Get the actual X-DATE from the display selection
                    if selected_xdate_display == "All Terms":
                        selected_xdate = "All Terms"
                    else:
                        selected_xdate = x_date_mapping.get(selected_xdate_display, selected_xdate_display)
                        ...
                        for idx, row in policy_rows.iterrows():
                            ...
                            # Include END within the term dates
                                ...
                                if term_eff_date <= trans_eff_date <= term_x_date:
                                    filtered_rows.append(idx)
                            # Include payment/reconciliation transactions within term based on Effective Date
                            elif trans_type in ["STMT", "VOID", "ADJ"] or "-STMT-" in str(row.get("Transaction ID", "")) or "-VOID-" in str(row.get("Transaction ID", "")):
                                # Check if Effective Date falls within term (same as other transaction types)
            ...
            if not policy_rows.empty:
                ...
                credit_col = get_mapped_column("Total Agent Comm") or "Total Agent Comm"
...
TO "New Column Name";
    ...
    # --- Policy Revenue Ledger Reports ---
        ...
        with st.sidebar:
            # Fix buttons outside expander for visibility
            st.markdown("### 🔧 Quick Fix")
            if st.button("🔨 Fix Detailed View Checkboxes", type="primary", use_container_width=True, key="fix_detailed_view_main"):
                # Clear only transaction reviews
                if 'prl_transaction_reviews' in st.session_state:
                    st.session_state.prl_transaction_reviews.clear()
                st.success("✅ Cleared transaction reviews!")
                st.rerun()
            
            with st.expander("🐛 Debug Info", expanded=False):
                st.write(f"**Rerun Count:** {st.session_state.rerun_count}")
                st.write("**Recent Reruns:**")
                for rerun in reversed(st.session_state.rerun_history[-5:]):
                    st.text(rerun)
                
                # Calculate rerun frequency
                if len(st.session_state.rerun_history) >= 2:
                    times = []
                    for rerun in st.session_state.rerun_history[-5:]:
                        if " at " in rerun:
                            try:
                                time_str = rerun.split(" at ")[1]
                    ...
                    if len(times) >= 2:
                        ...
                        st.write(f"**Avg time between reruns:** {avg_diff:.2f} seconds")
                        
                        if avg_diff < 0.5:
                            st.error("⚠️ RAPID RERUN DETECTED!")
                
                st.divider()
                st.write("**Review Status:**")
                st.write(f"Reviewed transactions: {len(st.session_state.get('prl_transaction_reviews', set()))}")
                st.write(f"Reviewed policies: {len(st.session_state.get('prl_reviewed_policies', set()))}")
                
                if st.button("Clear All Reviews", type="secondary", use_container_width=True, key="clear_all_debug"):
                    if 'prl_transaction_reviews' in st.session_state:
                        st.session_state.prl_transaction_reviews.clear()
                    if 'prl_reviewed_policies' in st.session_state:
                        st.session_state.prl_reviewed_policies.clear()
                    st.rerun()
        ...
        if all_data.empty:
            ...
            # IMPORTANT: Trim whitespace from policy numbers to avoid duplicates (same as Policy Revenue Ledger)
            if "Policy Number" in all_data.columns:
                all_data["Policy Number"] = all_data["Policy Number"].astype(str).str.strip()
            
            # Special handling for STMT transactions (same as Policy Revenue Ledger)
            # Find all STMT transactions that might have missing/different Client IDs
            stmt_transactions = all_data[
                all_data["Transaction ID"].astype(str).str.contains("-STMT-", na=False)
            ].copy()
            
            if not stmt_transactions.empty:
                # For each unique policy number with STMT transactions, ensure they're included
                for policy_num in stmt_transactions["Policy Number"].unique():
                    # Find the main client ID for this policy
                    policy_data = all_data[all_data["Policy Number"] == policy_num]
                    if not policy_data.empty and "Client ID" in policy_data.columns:
                        # Get the most common Client ID
                        client_ids = policy_data["Client ID"].dropna()
                        if not client_ids.empty:
                            main_client_id = client_ids.mode().iloc[0] if not client_ids.mode().empty else client_ids.iloc[0]
                            
                            # Update STMT transactions with missing Client IDs
                            missing_client_mask = (
                                (all_data["Policy Number"] == policy_num) & 
                                (all_data["Transaction ID"].astype(str).str.contains("-STMT-", na=False)) &
                                (all_data["Client ID"].isna())
                            )
                            if missing_client_mask.any():
                                all_data.loc[missing_client_mask, "Client ID"] = main_client_id
            # View mode toggle
            ...
            # Define function to apply custom transaction sorting
            def apply_transaction_sorting(df, view_mode):
                """Apply custom sorting to put STMT transactions at bottom, VOID at very bottom"""
                if view_mode == "Aggregated by Policy" or 'Transaction ID' not in df.columns:
                    return df
                
                # Create a sort key: 
                # 0 for regular transactions (NEW, RWL, END, etc.) and IMPORT
                # 1 for STMT transactions
                # 2 for VOID transactions
                def get_transaction_sort_priority(trans_id):
                    trans_id_str = str(trans_id)
                    if '-VOID-' in trans_id_str:
                        return 2
                    elif '-STMT-' in trans_id_str:
                        return 1
                    else:
                        return 0  # Regular transactions and IMPORT stay at top
                
                df = df.copy()
                df['_trans_sort_priority'] = df['Transaction ID'].apply(get_transaction_sort_priority)
                
                # Build sort columns
                sort_cols = []
                if 'Customer' in df.columns:
                    sort_cols.append('Customer')
                if 'Policy Number' in df.columns:
                    sort_cols.append('Policy Number')
                if 'Policy Term' in df.columns:
                    sort_cols.append('Policy Term')
                sort_cols.append('_trans_sort_priority')
                if 'Effective Date' in df.columns:
                    sort_cols.append('Effective Date')
                
                # Sort and drop temporary column
                df = df.sort_values(sort_cols, ascending=[True] * len(sort_cols))
                df = df.drop(columns=['_trans_sort_priority'])
            ...
            if view_mode == "Aggregated by Policy":
                ...
                if policy_number_col in working_data.columns and not working_data.empty:
                    ...
                    # Sort aggregated data by Customer then Policy Number
                    if 'Customer' in working_data.columns and 'Policy Number' in working_data.columns:
                        working_data = working_data.sort_values(['Customer', 'Policy Number'], ascending=[True, True])
            else:
                # Detailed view - show all transactions without aggregation
                working_data = all_data.copy()
                
                # Sort detailed data by Customer, Policy Number, Policy Term, then Effective Date
                sort_columns = []
                if 'Customer' in working_data.columns:
                    sort_columns.append('Customer')
                if 'Policy Number' in working_data.columns:
                    sort_columns.append('Policy Number')
                if 'Policy Term' in working_data.columns:
                    sort_columns.append('Policy Term')
                if 'Effective Date' in working_data.columns:
                    sort_columns.append('Effective Date')
                
                if sort_columns:
                    working_data = working_data.sort_values(sort_columns, ascending=[True] * len(sort_columns))
                
                # Apply custom transaction sorting
            ...
            # Formula: Policy Balance Due = "Total Agent Comm" - "Agent Paid Amount (STMT)"
            # Use Total Agent Comm to include broker fees
            if "Agent Paid Amount (STMT)" in working_data.columns and "Total Agent Comm" in working_data.columns:
                paid_amounts = pd.to_numeric(working_data["Agent Paid Amount (STMT)"], errors="coerce").fillna(0)
                commission_amounts = pd.to_numeric(working_data["Total Agent Comm"], errors="coerce").fillna(0)
            ...
            with stmt_col1:
                ...
                if 'Effective Date' in working_data.columns:
                    ...
                    # Use session state to get the default value if it exists
            ...
            with stmt_col2:
                if selected_month != "All Months":
                    ...
                    # Re-apply transaction type sorting for detailed view
                    month_data = apply_transaction_sorting(month_data, view_mode)
            ...
            # Initialize hidden/reviewed policies for aggregated view
            if 'prl_hidden_policies' not in st.session_state:
                st.session_state.prl_hidden_policies = set()
            
            if 'prl_reviewed_policies' not in st.session_state:
                st.session_state.prl_reviewed_policies = set()
            
            # Initialize individual transaction review tracking
            if 'prl_transaction_reviews' not in st.session_state:
                st.session_state.prl_transaction_reviews = set()  # Set of reviewed Transaction IDs
            
            # No longer apply hidden filters - using Reviewed checkboxes instead
            ...
            if view_mode == "Aggregated by Policy":
                # Policy-level columns for aggregated view
                default_columns = ["Customer", "Client ID", "Policy Number", "Policy Type", "Carrier Name", 
                                 "MGA Name", "Effective Date", "Policy Origination Date", "X-DATE", 
                                 "Total Agent Comm", "Agent Paid Amount (STMT)", "Policy Balance Due"]
            else:
                # Transaction-level columns for detailed view - comprehensive set for transaction analysis
                default_columns = ["Transaction ID", "Transaction Type", "Customer", "Policy Number", "Policy Term", "Policy Type", 
                                 "Carrier Name", "MGA Name", "Effective Date", "X-DATE", "Premium Sold", 
                                 "Policy Taxes & Fees", "Commissionable Premium", "Broker Fee", "Broker Fee Agent Comm",
                                 "Total Agent Comm", "Agent Paid Amount (STMT)", "Policy Balance Due"]
            
            available_default_columns = [col for col in default_columns if col in all_columns]
            
            # Check for default template for current view mode and load it
            default_template = None
            for name, data in st.session_state.prl_templates.items():
                if data.get("is_default", False) and data.get("view_mode", "Detailed Transactions") == view_mode:
                    default_template = name
                    break
            
            # Debug: Show template status
            if default_template:
                st.sidebar.info(f"🔍 Default template found: {default_template}")
            else:
                st.sidebar.warning("⚠️ No default template set")
            
            # Initialize selected columns in session state
            # Store the current view mode to detect changes
            if "prl_current_view_mode" not in st.session_state:
                st.session_state.prl_current_view_mode = view_mode
            
            # If view mode changed, reset columns to appropriate defaults
            if st.session_state.prl_current_view_mode != view_mode:
                st.session_state.prl_current_view_mode = view_mode
                if default_template:
            ...
            elif "prl_selected_columns" not in st.session_state:
                if default_template:
                    # Load the default template columns
                    template_data = st.session_state.prl_templates[default_template]
                    valid_template_columns = [col for col in template_data["columns"] if col in all_columns]
            ...
            with st.expander("🔧 Column Selection & Templates", expanded=False):
                ...
                # Add Load Default Template button if a default exists
                if default_template:
                    col1, col2, col3 = st.columns([1, 1, 2])
                    with col1:
                        if st.button("🔄 Load Default Template", type="primary", key="force_load_default"):
                            template_data = st.session_state.prl_templates[default_template]
                            template_columns = template_data.get("columns", [])
                            
                            # Show debug info in a container so it stays visible
                            debug_container = st.container()
                            with debug_container:
                                st.info("🔍 **Debug Info - Template Loading**")
                                st.write("**Template columns:**")
                                st.write(template_columns)
                                
                                valid_template_columns = [col for col in template_columns if col in all_columns]
                                st.write("**Valid columns found:**")
                                st.write(valid_template_columns)
                                
                                # Check for mismatches
                                missing_columns = [col for col in template_columns if col not in all_columns]
                                if missing_columns:
                                    st.warning(f"**Missing columns:** {missing_columns}")
                                
                                st.write("**First 20 available columns:**")
                                st.write(all_columns[:20])
                            
                            st.session_state.prl_selected_columns = valid_template_columns
                            # Clear the multiselect widget key to force refresh
                            if 'prl_column_multiselect' in st.session_state:
                                del st.session_state['prl_column_multiselect']
                            st.success(f"✅ Loaded {len(valid_template_columns)} columns from template: {default_template}")
                ...
                with col_selection_col1:
                    ...
                    if selected_columns != st.session_state.get('prl_selected_columns', []):
                ...
                with col_selection_col2:
                    ...
                    if st.button("💰 Financial Focus"):
                        financial_cols = ["Customer", "Policy Number", "Agency Estimated Comm/Revenue (CRM)", "Total Agent Comm", "Agent Paid Amount (STMT)", "Policy Balance Due"]
            ...
            with st.expander("💾 Template Management", expanded=False):
                ...
                with template_col2:
                    ...
                    if st.session_state.prl_templates:
                        # Filter templates by current view mode
                        view_templates = {name: data for name, data in st.session_state.prl_templates.items() 
                                        if data.get("view_mode", "Detailed Transactions") == view_mode}
                    ...
                    if st.session_state.prl_templates:
                        ...
                        if view_templates:
                            template_to_manage = st.selectbox(
                                ...
                                options=list(view_templates.keys()),
            ...
            if st.session_state.prl_templates:
                with st.expander("📋 Saved Templates", expanded=False):
                    # Filter templates by current view mode
                    current_view_templates = {name: data for name, data in st.session_state.prl_templates.items() 
                                            if data.get("view_mode", "Detailed Transactions") == view_mode}
                    
                    if current_view_templates:
                        st.info(f"📊 Showing templates for {view_mode} view")
                        for name, data in current_view_templates.items():
                        ...
                        st.info(f"No templates saved for {view_mode} view yet")
            
            # All hidden/reviewed transaction views removed - using checkboxes instead
            
            # Show reviewed transactions details if requested - Only for Detailed view
            if view_mode != "Aggregated by Policy" and st.session_state.get('show_reviewed_details', False):
                with st.expander("✅ Reviewed Policies" if view_mode == "Aggregated by Policy" else "✅ Reviewed Transactions", expanded=True):
                    # Check the appropriate session state based on view mode
                    has_reviewed_items = (
                        (view_mode == "Aggregated by Policy" and st.session_state.get('prl_reviewed_policies')) or
                        (view_mode != "Aggregated by Policy" and st.session_state.get('prl_reviewed_rows'))
                    )
                    
                    if has_reviewed_items:
                        # For aggregated view, we need to aggregate the reviewed data too
                        if view_mode == "Aggregated by Policy":
                            # Get the reviewed policies
                            if st.session_state.prl_reviewed_policies:
                                # Get all transactions for these policies
                                reviewed_df = all_data[all_data['Policy Number'].isin(st.session_state.prl_reviewed_policies)]
                                
                                # Apply the same aggregation logic as the main table
                                if 'Policy Number' in reviewed_df.columns:
                                    # Copy the aggregation logic from the main table
                                    agg_dict = {}
                                    ...
                                    descriptive_field_names = ["Customer", "Policy Type", "Carrier Name", "MGA Name", "Effective Date", 
                                                             ...
                                                             "Policy Checklist Complete", "STMT DATE"]
                                    for field_name in descriptive_field_names:
                                        if field_name in reviewed_df.columns:
                                            agg_dict[field_name] = 'first'
                                    ...
                                    for field_name in monetary_field_names:
                                        if field_name in reviewed_df.columns:
                                            reviewed_df[field_name] = pd.to_numeric(reviewed_df[field_name], errors='coerce').fillna(0)
                                            agg_dict[field_name] = 'sum'
                                    
                                    # Percentage fields - take first
                                    percentage_fields = ["Agent Comm %", "Policy Gross Comm %"]
                                    for field in percentage_fields:
                                    ...
                                    if "Agent Paid Amount (STMT)" in reviewed_df.columns and "Total Agent Comm" in reviewed_df.columns:
                                        reviewed_df["Policy Balance Due"] = pd.to_numeric(reviewed_df["Total Agent Comm"], errors="coerce").fillna(0) - pd.to_numeric(reviewed_df["Agent Paid Amount (STMT)"], errors="coerce").fillna(0)
                                    else:
                                        reviewed_df["Policy Balance Due"] = 0
                            else:
                                reviewed_df = pd.DataFrame()
                        else:
                            # Detailed view - just get the reviewed transactions
                            reviewed_df = all_data[all_data['Transaction ID'].astype(str).isin(st.session_state.prl_reviewed_rows)]
                        
                        # Show summary
                        st.markdown(f"**{len(reviewed_df)} reviewed {'policies' if view_mode == 'Aggregated by Policy' else 'transactions'}:**")
                        
                        if len(reviewed_df) > 0:
                            # Use the same columns as the Report Preview
                            if selected_columns:
                                # Filter to only include columns that exist in reviewed_df
                                available_cols = [col for col in selected_columns if col in reviewed_df.columns]
                                
                                # Replace "Agent Estimated Comm $" with "Total Agent Comm" if selected
                                if "Agent Estimated Comm $" in available_cols and "Total Agent Comm" in reviewed_df.columns:
                                    available_cols = ["Total Agent Comm" if col == "Agent Estimated Comm $" else col for col in available_cols]
                            else:
                                # Fallback to default columns if none selected
                                if view_mode == "Aggregated by Policy":
                                    display_cols = ['Transaction ID', 'Customer', 'Client ID', 'Policy Number', 'Policy Type', 
                                                  'Carrier Name', 'MGA Name', 'Effective Date', 'Policy Origination Date', 'X-DATE',
                                                  'Total Agent Comm', 'Agent Paid Amount (STMT)', 'Policy Balance Due']
                                else:
                                    display_cols = ['Transaction ID', 'Transaction Type', 'Customer', 'Policy Number', 'Policy Term', 'Policy Type', 
                                                  'Carrier Name', 'MGA Name', 'Effective Date', 'X-DATE', 'Premium Sold', 
                                                  'Policy Taxes & Fees', 'Commissionable Premium', 'Broker Fee', 'Broker Fee Agent Comm',
                                                  'Total Agent Comm', 'Agent Paid Amount (STMT)', 'Policy Balance Due']
                            ...
                            # Sort by Customer, Policy Number, and Policy Term for detailed view
                            sort_cols = []
                            if 'Customer' in reviewed_editable.columns:
                                sort_cols.append('Customer')
                            if 'Policy Number' in reviewed_editable.columns:
                                sort_cols.append('Policy Number')
                            if view_mode != "Aggregated by Policy" and 'Policy Term' in reviewed_editable.columns:
                                sort_cols.append('Policy Term')
                            if 'Effective Date' in reviewed_editable.columns:
                                sort_cols.append('Effective Date')
                            
                            if sort_cols:
                                reviewed_editable = reviewed_editable.sort_values(sort_cols, ascending=[True] * len(sort_cols))
                            ...
                            for col in ['Total Agent Comm', 'Agent Estimated Comm $', 'Agent Paid Amount (STMT)', 'Policy Balance Due']:
                            ...
                            # Use data editor for reviewed transactions with extra row padding
                            edited_reviewed = st.data_editor(
                                ...
                                num_rows="fixed",
                                height=(len(reviewed_editable) + 1) * 35 + 50  # Add extra row height
                            )
                        ...
                        with action_col1:
                            if st.button("✨ Clear Selected Reviews", type="primary", key="clear_selected_reviews"):
                                ...
                                if view_mode == "Aggregated by Policy":
                                    items_to_clear = edited_reviewed[edited_reviewed['Clear Review'] == True]['Policy Number'].tolist()
                                    if items_to_clear:
                                        # Remove from reviewed set
                                        for policy_num in items_to_clear:
                                            st.session_state.prl_reviewed_policies.discard(str(policy_num))
                                        st.session_state.show_reviewed_details = False
                                        st.rerun()
                                    else:
                                        st.warning("No policies selected to clear review")
                                else:
                                    items_to_clear = edited_reviewed[edited_reviewed['Clear Review'] == True]['Transaction ID'].astype(str).tolist()
                                    if items_to_clear:
                                        # Remove from reviewed set
                                        for trans_id in items_to_clear:
                                            st.session_state.prl_reviewed_rows.discard(trans_id)
                                        st.session_state.show_reviewed_details = False
                                        st.rerun()
                                    else:
                                        st.warning("No transactions selected to clear review")
                        ...
                        st.info("No reviewed transactions to display")
                        if st.button("Close", key="close_empty_reviewed"):
                            st.session_state.show_reviewed_details = False
                            st.rerun()
            ...
            with view_mode_col2:
                ...
                # Initialize the toggle state if not exists
                if 'prl_column_view_toggle' not in st.session_state:
                    st.session_state.prl_column_view_toggle = st.session_state.prl_column_view_mode
            ...
            if selected_columns and not working_data.empty:
                ...
                if "Agent Estimated Comm $" in valid_columns and "Total Agent Comm" in working_data.columns:
                    # Replace in the list
                    valid_columns = ["Total Agent Comm" if col == "Agent Estimated Comm $" else col for col in valid_columns]
                ...
                if valid_columns:
                    ...
                    # For aggregated view, we use Policy Number; for detailed view, we use Transaction ID
                    if view_mode == "Aggregated by Policy":
                        # Aggregated view uses Policy Number for tracking
                        if 'Policy Number' in display_data.columns:
                            selection_available = True
                        else:
                            selection_available = False
                    else:
                        # Detailed view uses Transaction ID for tracking
                        selection_available = 'Transaction ID' in display_data.columns
                    ...
                    if selection_available:
                        ...
                        if view_mode != "Aggregated by Policy" and 'Transaction ID' in editable_data.columns:
                            ...
                            reviewed_status = editable_data['Transaction ID'].apply(
                                lambda x: str(x) in st.session_state.prl_transaction_reviews
                            )
                            editable_data.insert(0, 'Reviewed', reviewed_status)
                        
                        # Add Reviewed checkbox column for Aggregated view only
                        if view_mode == "Aggregated by Policy" and 'Policy Number' in editable_data.columns:
                            # Create reviewed status for each policy
                            reviewed_status = editable_data['Policy Number'].apply(
                                lambda x: str(x) in st.session_state.prl_reviewed_policies
                            )
                            editable_data.insert(0, 'Reviewed', reviewed_status)
                        
                        # Add Type column with symbols if Transaction Type exists (only for detailed view)
                        if view_mode != "Aggregated by Policy" and 'Transaction Type' in editable_data.columns and 'Transaction ID' in editable_data.columns:
                        ...
                        if view_mode == "Aggregated by Policy" and 'Reviewed' in editable_data.columns:
                            column_config['Reviewed'] = st.column_config.CheckboxColumn(
                                'Reviewed',
                        ...
                        if view_mode != "Aggregated by Policy" and 'Policy Number' in editable_data.columns:
                            # Group by policy terms using Effective Date and X-DATE ranges
                            # This matches the logic in Policy Revenue Ledger
                            
                            # Track term groups for each row
                            editable_data['_term_group'] = ''
                            editable_data['_term_dates'] = ''
                            
                            # Process each unique policy number
                            unique_policies = editable_data['Policy Number'].unique()
                            all_term_groups = []
                            
                            for policy_num in unique_policies:
                                # Get all transactions for this policy
                                policy_mask = editable_data['Policy Number'] == policy_num
                                ...
                                if 'X-DATE' in policy_data.columns and 'Transaction Type' in policy_data.columns:
                                    ...
                                    if not term_defining.empty:
                                        # Remove duplicates based on Effective Date and X-DATE to avoid double-counting terms
                                        term_defining = term_defining.drop_duplicates(subset=['Effective Date', 'X-DATE'])
                                        ...
                                        # No terms found, group all transactions together
                                        term_name = f"{policy_num} - All Transactions"
                                        editable_data.loc[policy_mask, '_term_group'] = term_name
                                        all_term_groups.append(term_name)
                                else:
                                    # Missing required columns, group by policy only
                                    term_name = f"{policy_num} - All Transactions"
                                    editable_data.loc[policy_mask, '_term_group'] = term_name
                                    all_term_groups.append(term_name)
                            ...
                            for group_name in unique_groups:
                                group_data = editable_data[editable_data['_term_group'] == group_name]
                                if len(group_data) > 0:
                                    # Create subtotal row
                                    subtotal_row = {}
                                    
                                    # Copy structure from first row to get all columns
                                    ...
                                    term_dates = group_data['_term_dates'].iloc[0] if '_term_dates' in group_data.columns else ''
                                    if term_dates:
                                        subtotal_row['Customer'] = f'{len(group_data)} transactions ({term_dates})'
                                    else:
                                        subtotal_row['Customer'] = f'{len(group_data)} transactions'
                                    
                                    # Calculate numeric totals
                                    for col in numeric_columns:
                                        if col in group_data.columns:
                                            try:
                                                total = group_data[col].replace('', 0).fillna(0).astype(float).sum()
                                    ...
                                    last_position = editable_data.index.get_loc(group_data.index[-1])
                                    subtotal_rows.append((last_position, subtotal_row))
                            ...
                            for idx, (insert_position, subtotal_row) in enumerate(reversed(subtotal_rows)):
                                ...
                                after = editable_data.iloc[insert_position+1:]
                                
                                # Concatenate with subtotal in between
                            ...
                            editable_data.drop([col for col in cols_to_drop if col in editable_data.columns], axis=1, inplace=True)
                        
                        # Reorder columns based on selected columns order from template/multiselect
                        # Get the order from st.session_state.prl_selected_columns
                        if 'prl_selected_columns' in st.session_state and st.session_state.prl_selected_columns:
                            # Use template/selected column order
                            template_order = st.session_state.prl_selected_columns
                            
                            # Special columns that need to be at the beginning
                            special_cols = []
                            if 'Reviewed' in editable_data.columns:
                                special_cols.append('Reviewed')
                            if 'Group' in editable_data.columns:
                                special_cols.append('Group')
                            if 'Type →' in editable_data.columns:
                                special_cols.append('Type →')
                            
                            # Get columns in template order (excluding special columns)
                            template_cols = [col for col in template_order if col in editable_data.columns and col not in special_cols]
                            
                            # Get any remaining columns not in template
                            all_used_cols = special_cols + template_cols
                            remaining_cols = [col for col in editable_data.columns if col not in all_used_cols]
                            
                            # Final order: special columns + template order + remaining
                            reordered_columns = special_cols + template_cols + remaining_cols
                        else:
                            # Fallback to default important columns if no template
                            important_columns = []
                            
                            # Build important columns list based on what exists
                            if 'Reviewed' in editable_data.columns:
                                important_columns.append('Reviewed')
                            if 'Group' in editable_data.columns:
                                important_columns.append('Group')
                            if 'Type →' in editable_data.columns:
                                important_columns.append('Type →')
                            
                            # Add other important columns
                            important_columns.extend(['Transaction ID', 'Customer', 'Transaction Type', 'Policy Type', 'Policy Origination Date', 'Effective Date'])
                            
                            # Get columns that exist in the data
                            available_important = [col for col in important_columns if col in editable_data.columns]
                            remaining_columns = [col for col in editable_data.columns if col not in important_columns]
                            
                            # Reorder the dataframe
                            reordered_columns = available_important + remaining_columns
                        ...
                        with st.expander("💡 **Tips for Better Navigation**", expanded=False):
                            st.markdown("""
                            **To keep key columns visible while scrolling:**
                            ...
                            **Transaction Indicators:**
                            - 💰 = STMT reconciliation (light blue background)
                            - 🔴 = VOID transaction (light red background)
                            - ✓ = Individually reviewed transaction
                        ...
                        # Hide the _term_group column from display
                        if '_term_group' in editable_data.columns:
                            column_config['_term_group'] = st.column_config.Column(
                                '_term_group',
                                width=0,
                                disabled=True
                            )
                        ...
                        # Apply styling if Group column exists (only in detailed view with grouping)
                        if 'Group' in editable_data.columns:
                            # Drop _term_group before styling to avoid issues
                            if '_term_group' in editable_data.columns:
                                display_data = editable_data.drop(columns=['_term_group'])
                            else:
                        ...
                        edited_df = st.data_editor(
                            ...
                            height=display_height,
                            column_config=column_config,
                            disabled=[col for col in editable_data.columns if col != 'Reviewed'],  # Only Reviewed column is editable
                            hide_index=True,
                            key="prl_data_editor"
                        )
                        
                        # Only check for changes if the data_editor key exists in session state
                        # This indicates the widget has been interacted with
                        if st.session_state.get("prl_data_editor") is not None:
                            # Check if edited_rows exists (indicates actual edits were made)
                            editor_state = st.session_state.get("prl_data_editor")
                            if isinstance(editor_state, dict) and "edited_rows" in editor_state and editor_state["edited_rows"]:
                                # Debug tracking
                                st.session_state.rerun_history.append(f"Data editor changes detected at {datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
                                
                                # Process the edited rows
                                for row_idx, changes in editor_state["edited_rows"].items():
                                    if "Reviewed" in changes:
                                        new_reviewed = changes["Reviewed"]
                                        ...
                                        if row_idx < len(display_data):
                                            ...
                                            if view_mode == "Aggregated by Policy":
                                                policy_num = str(row_data['Policy Number'])
                                                if new_reviewed:
                                                    st.session_state.prl_reviewed_policies.add(policy_num)
                                                else:
                        ...
                        # Visual legend for transaction types (only for detailed view)
                        if view_mode != "Aggregated by Policy" and False:  # Temporarily disabled - remove entire section
                            st.markdown("---")
                            review_col1, review_col2, review_col3, review_col4 = st.columns(4)
                            
                            with review_col1:
                                if st.button("✓ Mark Transactions Reviewed", type="secondary", key="mark_individual_reviewed"):
                                    # Get selected rows
                                    ...
                                    if 'Group' in selected_rows.columns:
                                        ...
                                        if not subtotal_selections.empty:
                                            ...
                                            term_groups = []
                                            for _, row in subtotal_selections.iterrows():
                                                # Extract term name from subtotal Transaction ID
                                                trans_id = row['Transaction ID']
                                                if trans_id.startswith('SUBTOTAL: '):
                                                    term_name = trans_id.replace('SUBTOTAL: ', '')
                                                    term_groups.append(term_name)
                                            
                                            # Find all transactions in selected term groups
                                            items_to_review = []
                                            if '_term_group' in editable_data.columns:
                                            ...
                                            # Remove duplicates
                                            items_to_review = list(set(items_to_review))
                                            
                                            if items_to_review:
                                                for trans_id in items_to_review:
                                                    st.session_state.prl_transaction_reviews.add(str(trans_id))
                                                st.success(f"✓ Marked {len(items_to_review)} individual transactions as reviewed")
                                                st.rerun()
                                            else:
                                                st.warning("No transactions found in selected terms")
                                        else:
                                            # No subtotal rows selected, use regular transaction selection
                                            selected_trans = selected_rows['Transaction ID'].tolist()
                                            if selected_trans:
                                                # Add to individually reviewed set
                                                for trans_id in selected_trans:
                                                    st.session_state.prl_transaction_reviews.add(str(trans_id))
                                                st.success(f"✓ Marked {len(selected_trans)} individual transactions as reviewed")
                                                st.rerun()
                                            else:
                                                st.warning("No transactions selected")
                                    else:
                                        # No Group column, use regular selection
                                        selected_trans = selected_rows['Transaction ID'].tolist()
                                        if selected_trans:
                                            # Add to individually reviewed set
                                            for trans_id in selected_trans:
                            ...
                            with review_col2:
                                if st.button("✗ Clear Transaction Reviews", key="clear_individual_reviews"):
                                    # Get selected rows
                                    selected_rows = edited_df[edited_df['Select'] == True]
                                    ...
                                    if 'Group' in selected_rows.columns:
                                        ...
                                        if not subtotal_selections.empty:
                                            ...
                                            # Remove duplicates
                                            items_to_clear = list(set(items_to_clear))
                                            
                                            if items_to_clear:
                                                for trans_id in items_to_clear:
                                                    st.session_state.prl_transaction_reviews.discard(str(trans_id))
                                                st.success(f"✗ Cleared review status for {len(items_to_clear)} transactions")
                                                st.rerun()
                                            else:
                                                st.warning("No transactions found in selected terms")
                                        else:
                                            ...
                                            selected_trans = selected_rows['Transaction ID'].tolist()
                                            if selected_trans:
                                                # Remove from individually reviewed set
                                                for trans_id in selected_trans:
                                                    st.session_state.prl_transaction_reviews.discard(str(trans_id))
                                                st.success(f"✗ Cleared review status for {len(selected_trans)} transactions")
                                                st.rerun()
                                            else:
                                                st.warning("No transactions selected")
                                    else:
                            ...
                            with review_col3:
                                # Show count of reviewed transactions
                                reviewed_count = len(st.session_state.prl_transaction_reviews)
                                st.info(f"📊 {reviewed_count} transactions individually reviewed")
                    ...
                    export_metadata = {
                        ...
                        "Balance Due Formula": "Total Agent Comm - Agent Paid Amount (STMT)",
