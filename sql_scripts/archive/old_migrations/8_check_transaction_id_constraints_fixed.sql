-- =====================================================
-- Diagnostic Script for Transaction ID Issues (Fixed)
...
-- 5. Look for recent transactions (without using created_at)
SELECT 
    "Transaction ID",
    "Customer",
    "STMT DATE",
...
WHERE 
    ...
    OR is_reconciliation_entry = TRUE
ORDER BY 
    "_id" DESC
LIMIT 10;

-- 6. Check all columns in policies table to see what's available
SELECT 
    column_name
FROM 
    information_schema.columns
...
-- 7. Test inserting a reconciliation transaction directly
