                ...
                if valid_columns:
                    ...
                    with export_col3:
                        ...
                        with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                            ...
                            # Format numeric columns - but preserve subtotal formatting
                            for col in all_numeric_columns:
                                if col in excel_export_data.columns:
                                    # For detailed view with subtotals, we need to handle subtotal rows differently
                                    if view_mode != "Aggregated by Policy" and 'Group' in excel_export_data.columns:
                                        # Convert column to numeric but preserve subtotal rows
                                        for idx in excel_export_data.index:
                                            # Check if this is a subtotal row
                                            if excel_export_data.loc[idx, 'Group'] == '=':
                                                # For subtotal rows, check if value is a formatted currency string
                                                val = excel_export_data.loc[idx, col]
                                                if isinstance(val, str) and val.startswith('$'):
                                                    # Remove $ and commas, then convert to float
                                                    try:
                                                        numeric_val = float(val.replace('$', '').replace(',', ''))
                                                        excel_export_data.loc[idx, col] = numeric_val
                                                    except:
                                                        excel_export_data.loc[idx, col] = 0.0
                                            else:
                                                # For regular rows, convert normally
                                                excel_export_data.loc[idx, col] = pd.to_numeric(excel_export_data.loc[idx, col], errors='coerce')
                                    else:
                                        # For aggregated view, convert normally
