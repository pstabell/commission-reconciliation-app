...
TO "New Column Name";
    ...
    # --- Policy Revenue Ledger Reports ---
        ...
        if all_data.empty:
            ...
            # Show reviewed transactions indicator
            if 'prl_reviewed_rows' in st.session_state and len(st.session_state.prl_reviewed_rows) > 0:
                review_col1, review_col2, review_col3 = st.columns([2, 1, 1])
                with review_col1:
                    st.success(f"✅ {len(st.session_state.prl_reviewed_rows)} transactions marked as reviewed")
                with review_col2:
                    if st.button("View Reviewed", key="view_reviewed_rows"):
                        st.session_state.show_reviewed_details = True
                with review_col3:
                    if st.button("Clear All Reviews", key="clear_all_reviews"):
                        st.session_state.prl_reviewed_rows.clear()
                        st.success("Cleared all review marks")
                        st.rerun()
            ...
            if selected_columns and not working_data.empty:
                ...
                if valid_columns:
                    ...
                    if 'Transaction ID' in display_data.columns:
                        ...
                        button_col1, button_col2, button_col3, button_col4 = st.columns([1.5, 1.5, 1.5, 2])
