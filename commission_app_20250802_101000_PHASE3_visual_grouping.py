                ...
                if not recon_entries.empty:
                    ...
                    if not filtered_recon.empty:
                        ...
                        if view_mode == "By Batch" and 'reconciliation_id' in filtered_recon.columns:
                            ...
                            display_columns = ['Edit', 'Transaction ID', 'Transaction Type', 'Customer', 'Client ID', 'Policy Number', 'Policy Type', 'Carrier Name', 'Effective Date', 'STMT DATE']
            ...
            if selected_columns and not working_data.empty:
                ...
                if valid_columns:
                    ...
                    if selection_available:
                        ...
                        # Apply visual grouping by policy term (only in detailed view)
                        if view_mode != "Aggregated by Policy" and 'Policy Number' in editable_data.columns and 'Policy Term' in editable_data.columns:
                            # Create a style function for alternating policy term groups
                            def get_term_group_style():
                                # Group by Policy Number + Policy Term to identify groups
                                editable_data['_group_key'] = editable_data['Policy Number'].astype(str) + '_' + editable_data['Policy Term'].astype(str)
                                
                                # Assign group numbers
                                unique_groups = editable_data['_group_key'].unique()
                                group_map = {group: idx for idx, group in enumerate(unique_groups)}
                                editable_data['_group_num'] = editable_data['_group_key'].map(group_map)
                                
                                # Create style based on group number (alternate colors)
                                styles = []
                                for _, row in editable_data.iterrows():
                                    if row['_group_num'] % 2 == 0:
                                        # Even groups - light blue background
                                        styles.append({
                                            'background-color': 'rgba(173, 216, 230, 0.15)',  # Light blue with transparency
                                            'border-bottom': '1px solid rgba(173, 216, 230, 0.3)'
                                        })
                                    else:
                                        # Odd groups - keep default
                                        styles.append({
                                            'border-bottom': '1px solid rgba(128, 128, 128, 0.2)'
                                        })
                                
                                # Clean up temporary columns
                                editable_data.drop(['_group_key', '_group_num'], axis=1, inplace=True)
                                
                                return styles
                            
                            # Note: Streamlit's data_editor doesn't support row-level styling yet
                            # So we'll add a visual indicator column instead
                            # Group by Policy Number + Policy Term
                            editable_data['_term_group'] = editable_data['Policy Number'].astype(str) + ' - Term ' + editable_data['Policy Term'].astype(str)
                            ...
                            column_config['Group'] = st.column_config.TextColumn(
                                'Group',
                                help="◐ = Even policy term group, ○ = Odd policy term group",
                                width="small"
                            )
                            
                            # Clean up temporary column
                            editable_data.drop('_term_group', axis=1, inplace=True)
