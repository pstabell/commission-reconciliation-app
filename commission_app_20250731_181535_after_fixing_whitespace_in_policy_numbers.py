            ...
            if selected_columns and not working_data.empty:
                ...
                if valid_columns:
                    ...
                    if 'Transaction ID' in display_data.columns:
                        ...
                        # Add Reviewed checkbox column after Hide
                        # Check if transactions are in reviewed set
                        reviewed_status = editable_data['Transaction ID'].apply(
                            lambda x: str(x) in st.session_state.prl_reviewed_rows
                        )
                        ...
                        # Add column width configuration for the important columns
                        column_config['Hide'] = st.column_config.CheckboxColumn(
                            'Hide',
                            help="Select rows to hide",
                            default=False,
                        ...
                        with st.expander("💡 **Tips for Better Navigation**", expanded=False):
                            ...
                            **To keep key columns visible while scrolling:**
                            ...
                            2. **Column Order**: We've moved Hide, Reviewed, Transaction ID, Customer, Transaction Type, and Policy Type to the front
                            3. **Shift + Scroll**: Hold Shift while scrolling to move horizontally
                            4. **Review Progress**: Check/uncheck the Reviewed column and click "Save Reviewed" to track your progress
                        ...
                        edited_df = st.data_editor(
                            ...
                            height=display_height,
                            column_config=column_config,
                            disabled=[col for col in editable_data.columns if col not in ['Hide', 'Reviewed']],  # Hide and Reviewed columns are editable
                            hide_index=True,
                            key="prl_data_editor"
                        )
                        ...
                        with button_col2:
                            if st.button("✅ Save Reviewed", type="secondary", key="save_reviewed_rows"):
                                # Update reviewed rows based on checkbox states
                                reviewed_trans = edited_df[edited_df['Reviewed'] == True]['Transaction ID'].tolist()
                                unreviewed_trans = edited_df[edited_df['Reviewed'] == False]['Transaction ID'].tolist()
                                
                                # Add reviewed transactions
                                for trans_id in reviewed_trans:
                                    st.session_state.prl_reviewed_rows.add(str(trans_id))
                                
                                # Remove unreviewed transactions
                                for trans_id in unreviewed_trans:
                                    st.session_state.prl_reviewed_rows.discard(str(trans_id))
                                
                                st.success(f"✅ Saved review status for {len(edited_df)} transactions")
                                st.rerun()
