...
TO "New Column Name";
    ...
    # --- Policy Revenue Ledger Reports ---
        ...
        if all_data.empty:
            ...
            if view_mode == "Aggregated by Policy":
                # Policy-level columns for aggregated view - include Transaction ID for selection functionality
                default_columns = ["Transaction ID", "Customer", "Client ID", "Policy Number", "Policy Type", "Carrier Name", 
            ...
            if st.session_state.get('show_hidden_details', False):
                with st.expander("🙈 Hidden Policies" if view_mode == "Aggregated by Policy" else "🙈 Hidden Transactions", expanded=True):
                    ...
                    if view_mode == "Aggregated by Policy":
                        # Get the hidden transactions from all_data
                        hidden_raw = all_data[all_data['Transaction ID'].isin(st.session_state.prl_hidden_rows)]
                        
                        if not hidden_raw.empty:
                            # Get unique policy numbers from hidden transactions
                            hidden_policies = hidden_raw['Policy Number'].unique()
                            # Get all transactions for these policies (not just the hidden ones)
                            hidden_df = all_data[all_data['Policy Number'].isin(hidden_policies)]
            ...
            if st.session_state.get('show_reviewed_details', False):
                with st.expander("✅ Reviewed Policies" if view_mode == "Aggregated by Policy" else "✅ Reviewed Transactions", expanded=True):
                    if 'prl_reviewed_rows' in st.session_state and st.session_state.prl_reviewed_rows:
                        # For aggregated view, we need to aggregate the reviewed data too
                        if view_mode == "Aggregated by Policy":
                            # Get the reviewed transactions from all_data
                            reviewed_raw = all_data[all_data['Transaction ID'].astype(str).isin(st.session_state.prl_reviewed_rows)]
                            
                            if not reviewed_raw.empty:
                                # Get unique policy numbers from reviewed transactions
                                reviewed_policies = reviewed_raw['Policy Number'].unique()
                                # Get all transactions for these policies (not just the reviewed ones)
                                reviewed_df = all_data[all_data['Policy Number'].isin(reviewed_policies)]
            ...
            if selected_columns and not working_data.empty:
                ...
                if valid_columns:
                    ...
                    # For aggregated view, ensure Transaction ID is available even if not selected for display
                    if view_mode == "Aggregated by Policy" and 'Transaction ID' not in display_data.columns and 'Transaction ID' in working_data.columns:
                        # Add Transaction ID temporarily for the selection functionality
                        display_data_with_id = display_data.copy()
                        display_data_with_id['Transaction ID'] = working_data.loc[display_data.index, 'Transaction ID']
                        transaction_id_available = True
                    else:
                        display_data_with_id = display_data
                        transaction_id_available = 'Transaction ID' in display_data.columns
                    
                    if transaction_id_available:
                        # Create a copy for editing
                        editable_data = display_data_with_id.copy()
                        ...
                        with button_col1:
                            if st.button("🙈 Hide Selected", type="primary", key="hide_selected_rows"):
                                ...
                                rows_to_hide = edited_df[edited_df['Select'] == True]['Transaction ID'].tolist()
                                if rows_to_hide:
                                    st.session_state.prl_hidden_rows.update(rows_to_hide)
                                    st.success(f"✅ Hidden {len(rows_to_hide)} transactions")
                                    st.rerun()
                                else:
                        ...
                        with button_col2:
                            if st.button("✅ Mark as Reviewed", type="secondary", key="mark_reviewed_rows"):
                                ...
                                if selected_trans:
                                    ...
                                    st.success(f"✅ Marked {len(selected_trans)} transactions as reviewed")
                                    st.rerun()
                                else:
                                    st.warning("No rows selected to mark as reviewed")
