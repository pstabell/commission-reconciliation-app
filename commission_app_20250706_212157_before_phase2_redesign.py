...
def calculate_formula_values(df):
    """
    Add calculated formula columns to dataframe for display.
    Creates new columns with '_calc' suffix to show formula results.
    """
    if df.empty:
        return df
    ...
    # Calculate Agency Estimated Comm/Revenue (Formula)
    df['Agency Comm (Formula)'] = df.apply(
        lambda row: (
    ...
    # Calculate Agent Estimated Comm $ (Formula)
    df['Agent Comm (Formula)'] = df.apply(
        lambda row: (
            row['Agency Comm (Formula)'] * get_agent_rate(row) / 100
        ),
        axis=1
    )
    
    # Calculate variance columns
    df['Agency Comm Variance'] = df.apply(
        lambda row: (
            float(row.get('Agency Estimated Comm/Revenue (CRM)', 0) or 0) - row['Agency Comm (Formula)']
        ),
        axis=1
    )
    
    df['Agent Comm Variance'] = df.apply(
        lambda row: (
            float(row.get('Agent Estimated Comm $', 0) or 0) - row['Agent Comm (Formula)']
        ),
        axis=1
    )
    
    # Round all calculated columns to 2 decimal places
    formula_cols = ['Agency Comm (Formula)', 'Agent Comm (Formula)', 'Agency Comm Variance', 'Agent Comm Variance']
...
def main():
    ...
    # --- All Policies in Database ---
        ...
        if not all_data.empty:
            ...
            # Add formula columns for display
            paginated_data_with_formulas = calculate_formula_values(paginated_data)
            
            st.write(f"Showing records {start_idx + 1} to {end_idx} of {len(all_data)}")
            
            # Show formula columns info
            st.info("📊 **Formula Columns**: 'Agency Comm (Formula)' and 'Agent Comm (Formula)' are auto-calculated. 'Variance' columns show differences between manual entries and formulas.")
            ...
            numeric_cols = [
                ...
                'Agency Comm (Formula)',
                'Agent Comm (Formula)',
                'Agency Comm Variance',
                'Agent Comm Variance'
            ]
            
            for col in numeric_cols:
                if col in paginated_data_with_formulas.columns:
                    if 'Formula' in col:
                        # Style formula columns differently
                        column_config[col] = st.column_config.NumberColumn(
                            col,
                            format="%.2f",
                            help="Auto-calculated based on formula"
                        )
                    elif 'Variance' in col:
                        # Style variance columns with color coding
                        column_config[col] = st.column_config.NumberColumn(
                            col,
                            format="%.2f",
                            help="Difference between manual entry and formula"
                        )
                    else:
                        column_config[col] = st.column_config.NumberColumn(
                            col,
            ...
            # Add column order to show formulas next to manual entries
            column_order = []
            for col in paginated_data_with_formulas.columns:
                column_order.append(col)
                # Insert formula columns after their corresponding manual columns
                if col == 'Agency Estimated Comm/Revenue (CRM)':
                    if 'Agency Comm (Formula)' in paginated_data_with_formulas.columns:
                        column_order.append('Agency Comm (Formula)')
                    if 'Agency Comm Variance' in paginated_data_with_formulas.columns:
                        column_order.append('Agency Comm Variance')
                elif col == 'Agent Estimated Comm $':
                    if 'Agent Comm (Formula)' in paginated_data_with_formulas.columns:
                        column_order.append('Agent Comm (Formula)')
                    if 'Agent Comm Variance' in paginated_data_with_formulas.columns:
                        column_order.append('Agent Comm Variance')
            
            # Remove duplicates while preserving order
            column_order = list(dict.fromkeys(column_order))
            
            # Reorder dataframe
            paginated_data_with_formulas = paginated_data_with_formulas[column_order]
            
            # Display the data in a scrollable table
            # Note: Formula columns will be visually distinct through column headers
            st.dataframe(
                paginated_data_with_formulas,
                use_container_width=True,
                height=600,
                column_config=column_config
            )
            ...
            with col1:
                st.write("**📄 CSV Export**")
                csv = paginated_data_with_formulas.to_csv(index=False)
                st.download_button(
            ...
            with col2:
                # Add formulas to all data for export
                all_data_with_formulas = calculate_formula_values(all_data)
                csv_all = all_data_with_formulas.to_csv(index=False)
                st.write("**📄 CSV Export**")
                st.download_button(
                    label="📥 All Data CSV",
                    data=csv_all,
