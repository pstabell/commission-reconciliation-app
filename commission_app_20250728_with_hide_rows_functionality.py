    ...
    # --- Policy Revenue Ledger Reports ---
        ...
        if all_data.empty:
            ...
            if st.session_state.prl_hidden_rows and 'Transaction ID' in working_data.columns:
                ...
                # Show hidden rows indicator with unhide option
                if hidden_count > 0:
                    hide_col1, hide_col2, hide_col3 = st.columns([2, 1, 1])
                    with hide_col1:
                        st.warning(f"🙈 {hidden_count} rows hidden from view")
                    with hide_col2:
                    ...
                    if st.session_state.get('show_hidden_details', False):
                        with st.expander("Hidden Transactions", expanded=True):
                            ...
                            # Style the hidden transactions
                            styled_hidden = style_special_transactions(hidden_df[available_cols])
                            st.dataframe(styled_hidden, use_container_width=True)
                            
                            # Unhide options
                            unhide_col1, unhide_col2 = st.columns(2)
                            with unhide_col1:
            ...
            if selected_columns and not working_data.empty:
                ...
                if valid_columns:
                    ...
                    # Add hide functionality below the dataframe
                    if 'Transaction ID' in display_data.columns and len(display_data) > 0:
                        with st.expander("🙈 Hide Rows", expanded=False):
                            st.markdown("Select transaction IDs to hide from the view:")
                            
                            # Create columns for checkboxes (5 per row)
                            num_cols = 5
                            rows_to_hide = []
                            
                            # Get unique transaction IDs (excluding empty rows)
                            transaction_ids = display_data['Transaction ID'].dropna().unique()
                            
                            # Create checkbox grid
                            for i in range(0, len(transaction_ids), num_cols):
                                cols = st.columns(num_cols)
                                for j, col in enumerate(cols):
                                    if i + j < len(transaction_ids):
                                        trans_id = transaction_ids[i + j]
                                        with col:
                                            if st.checkbox(
                                                f"{trans_id}", 
                                                key=f"hide_checkbox_{trans_id}",
                                                value=trans_id in st.session_state.prl_hidden_rows
                                            ):
                                                rows_to_hide.append(trans_id)
                            
                            # Update hidden rows
                            if st.button("Apply Hide Selection", type="primary"):
                                st.session_state.prl_hidden_rows = set(rows_to_hide)
                                st.rerun()
