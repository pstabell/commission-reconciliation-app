                ...
                if valid_columns:
                    ...
                    if selection_available:
                        ...
                        if view_mode != "Aggregated by Policy" and 'Policy Number' in editable_data.columns:
                            # For now, just group by Policy Number if Policy Term is not meaningful
                            # This will at least show where one policy ends and another begins
                            if 'Policy Term' in editable_data.columns:
                                # Check if Policy Term has meaningful values
                                unique_terms = editable_data['Policy Term'].dropna().unique()
                                if len(unique_terms) > 1:
                                    # Policy Term has multiple values, use it for grouping
                                    editable_data['_policy_term_clean'] = editable_data['Policy Term'].fillna('1').astype(str)
                                    editable_data['_term_group'] = editable_data['Policy Number'].astype(str) + ' - Term ' + editable_data['_policy_term_clean']
                                else:
                                    # Policy Term is not meaningful, just group by Policy Number
                                    editable_data['_term_group'] = editable_data['Policy Number'].astype(str)
                            else:
                                # No Policy Term column, just group by Policy Number
                                editable_data['_term_group'] = editable_data['Policy Number'].astype(str)
                            
                            # Get unique groups in order
                            unique_groups = []
                            seen = set()
                            for group in editable_data['_term_group']:
                                if group not in seen:
                                    unique_groups.append(group)
                                    seen.add(group)
                            
                            # Create group indicator column
                            group_indicators = []
                            current_group = None
                            for _, row in editable_data.iterrows():
                                if row['_term_group'] != current_group:
                                    current_group = row['_term_group']
                                    # Find group number
                            ...
                            column_config['Group'] = st.column_config.TextColumn(
                                'Group',
                                help="◐ = Even group, ○ = Odd group, ▬ = Subtotal",
                                width="small"
                            )
                            ...
                            # Group data and create subtotals
                            ...
                            # Clean up temporary columns
                            cols_to_drop = ['_term_group']
                            if '_policy_term_clean' in editable_data.columns:
                                cols_to_drop.append('_policy_term_clean')
                            editable_data.drop(cols_to_drop, axis=1, inplace=True)
                        ...
                        with st.expander("💡 **Tips for Better Navigation**", expanded=False):
                            ...
                            **Visual Grouping:**
                            - ◐ = Even-numbered policy term groups (helps distinguish between terms)
                            - ○ = Odd-numbered policy term groups
                            - ▬ = Subtotal row for the policy term group
                            - The Group indicator appears only on the first transaction of each policy term
                            - Subtotal rows show totals for Premium Sold, Commissions, and Balance Due
                        ...
                        with button_col1:
                            if st.button("🙈 Hide Selected", type="primary", key="hide_selected_rows"):
                                ...
                                if view_mode == "Aggregated by Policy":
                                    ...
                                    # Use Transaction ID for detailed view
                                    items_to_hide = edited_df[edited_df['Select'] == True]['Transaction ID'].tolist()
                                    if items_to_hide:
                        ...
                        with button_col2:
                            if st.button("✅ Mark as Reviewed", type="secondary", key="mark_reviewed_rows"):
                                ...
                                if view_mode == "Aggregated by Policy":
                                    ...
                                    # Use Transaction ID for detailed view
                                    selected_items = edited_df[edited_df['Select'] == True]['Transaction ID'].tolist()
                                    if selected_items:
                        ...
                        if view_mode != "Aggregated by Policy":
                            ...
                            with review_col1:
                                if st.button("✓ Mark Transactions Reviewed", type="secondary", key="mark_individual_reviewed"):
                                    # Get selected rows
                                    selected_trans = edited_df[edited_df['Select'] == True]['Transaction ID'].tolist()
                                    if selected_trans:
