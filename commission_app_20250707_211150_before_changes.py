...
def get_custom_css():
    ...
    return """<style>        /* Remove default padding and maximize main block width */
        ...
        /* Add yellow border to New Policy Premium input */
...
def main():
                        ...
                        if not edit_results.empty:
                            ...
                            if st.session_state.get('show_edit_modal', False):
                                ...
                                with st.form("edit_transaction_form"):
                                    ...
                                    st.markdown("#### Broker Fee / Carrier Taxes & Fees")
                                    col5, col6 = st.columns(2)
                                    ...
                                    with col6:
                                        ...
                                        commissionable_premium = updated_data['Premium Sold'] - updated_data['Policy Taxes & Fees']
                                        st.number_input(
                                            'Commissionable Premium ($)',
                                    ...
                                    with col7:
                                        for field in left_commission_fields:
                                            if field == 'Broker Fee Agent Comm':
                                                # Calculate broker fee agent commission (always 50%)
                                                broker_fee_agent_comm = updated_data.get('Broker Fee', 0) * 0.50
                                                st.number_input(
                                                    field,
                                                    value=broker_fee_agent_comm,
                                                    format="%.2f",
                                                    key=f"modal_{field}_display",
                                                    disabled=True,
                                                    help="50% of broker fee"
                                                )
                                                updated_data[field] = broker_fee_agent_comm
                                    ...
                                    with col8:
                                        ...
                                        broker_fee_agent = updated_data.get('Broker Fee Agent Comm', 0)
                                        ...
                                        try:
                                            ...
                                            broker_fee_agent = float(broker_fee_agent) if broker_fee_agent else 0.0
                                        except:
                                            ...
                                            if field in modal_data.keys():
                                                if field == 'Agency Estimated Comm/Revenue (CRM)':
                                                    ...
                                                    st.number_input(
                                                        ...
                                                        key=f"modal_{field}_display",
                                                        disabled=True,
                                                        help=f"Formula: Commissionable Premium × Policy Gross Comm % = ${commissionable_prem:.2f} × {gross_comm_pct:.1f}% = ${agency_comm:.2f}"
                                                    )
                                        ...
                                        for field in left_other_fields:
                                            if field in editable_other_fields and field in modal_data.keys():
                                                if field == 'Policy Type':
                                                    ...
                                                    # Add option for custom types if allowed
                                                    type_options = active_types.copy()
                                                    if policy_types_config.get('allow_custom', True):
                                                        type_options.append("+ Add New Type...")
                                                    
                                                    # Get current value
                                                    current_policy_type = modal_data.get(field, '')
                                                    
                                                    # Ensure current value is in options
                                                    if current_policy_type and current_policy_type not in type_options and current_policy_type != "+ Add New Type...":
                                                        type_options.insert(0, current_policy_type)
                                                    
                                                    # Create key for managing state
                                                    policy_type_key = f"modal_{field}_select"
                                                    new_type_key = f"modal_{field}_new"
                                                    
                                                    # Initialize session state
                                                    if policy_type_key not in st.session_state:
                                                        st.session_state[policy_type_key] = current_policy_type if current_policy_type else type_options[0]
                                                    
                                                    selected_type = st.selectbox(
                                                        field,
                                                        options=type_options,
                                                        index=type_options.index(st.session_state[policy_type_key]) if st.session_state[policy_type_key] in type_options else 0,
                                                        key=policy_type_key
                                                    )
                                                    
                                                    # If "Add New Type" is selected, show input field
                                                    if selected_type == "+ Add New Type...":
                                                        new_type = st.text_input(
                                                            "Enter new policy type:",
                                                            key=new_type_key,
                                                        ...
                                                        if new_type:
                                                            ...
                                                            add_policy_type(new_type)
                                                            st.success(f"Added '{new_type}' to policy types")
                                                            # Update the selection to the new type
                                                            st.session_state[policy_type_key] = new_type
                                                            updated_data[field] = new_type
                                                        else:
                                                            # Keep the original value if no new type entered
                                                            updated_data[field] = current_policy_type
                                                    else:
                                                        updated_data[field] = selected_type
                                                else:
                                                    updated_data[field] = st.text_input(
                                                        field,
                                                        value=str(modal_data.get(field, '')) if modal_data.get(field) is not None else '',
    ...
    # --- Add New Policy Transaction ---
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            premium_sold = st.number_input("New Policy Premium", value=-1000000.00, format="%.2f", step=100.0, key="premium_sold_input")
        with col2:
        ...
        with st.form("add_policy_form"):
            ...
            with col1:
                ...
                # Add option for custom type if allowed
                if allow_custom:
                    policy_type_options = active_types + ["+ Add New Type..."]
                else:
                    policy_type_options = active_types
                ...
                policy_type = st.selectbox("Policy Type", policy_type_options, index=default_index)
                
                # Handle custom type entry
                if policy_type == "+ Add New Type...":
                    new_policy_type = st.text_input("Enter New Policy Type Name", key="new_policy_type_add")
                    if new_policy_type:
                        if st.button("Add Type", key="add_policy_type_inline"):
                            success, message = add_policy_type(new_policy_type)
                            if success:
                                st.success(message)
                                # Reset to use the new type
                                st.session_state['new_policy_type_added'] = new_policy_type
                                st.rerun()
                            else:
                                st.error(message)
                        policy_type = new_policy_type  # Use the new type for form submission
                
                # Check if we just added a new type
                if 'new_policy_type_added' in st.session_state:
                    policy_type = st.session_state['new_policy_type_added']
                    del st.session_state['new_policy_type_added']
            ...
            with col2:
                ...
                # Calculate and display commissionable premium
                # Use the premium from the calculators above (either endorsement or new policy)
                if 'premium_sold_calc' in locals() and premium_sold_calc != new_premium - existing_premium:
                ...
                st.text_input("Commissionable Premium ($)", value=f"{commissionable_premium:.2f}", disabled=True, help="Premium minus Carrier Taxes & Fees")
            
            # Commission Details Section
            st.subheader("Commission Details")
            col1, col2 = st.columns(2)
            with col1:
                # Use gross comm from calculator if available
                policy_gross_comm_value = policy_gross_comm_percent if 'policy_gross_comm_percent' in locals() else 0.0
                policy_gross_comm_input = st.number_input("Policy Gross Comm %", value=policy_gross_comm_value, format="%.2f", min_value=0.0, max_value=100.0)
            ...
            with col1:
                ...
                x_date = st.date_input("X-DATE", value=datetime.date.today() + datetime.timedelta(days=180), format="MM/DD/YYYY", help="Expiration date")
